...
describe("validateDate", () => {
  ...
  it('should return null for a valid date', () => {
    expect(validateDate('2023-2-30')).toBeNull();
  });
  ...
  it("should return an error for year greater than current year", () => {
    expect(validateDate(`${currentYear + 1}-01-01`)).toBe(
      `Year must be between 1000 and ${currentYear}.`
    );
  });
  ...
  it("should return an error for day 0", () => {
    expect(validateDate("2023-10-00")).toBe(
      "Invalid day for the selected month."
    );
  });

  it("should return an error for day 32", () => {
    expect(validateDate("2023-10-32")).toBe(
      "Invalid day for the selected month."
    );
  });
